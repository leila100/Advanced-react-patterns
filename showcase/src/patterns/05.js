...
const MediumClap = ({ children, onClap, values = null, style: userStyles = {}, className }) => {
  ...
  //   used to make sure the onClap function is not run until user claps
  ...
  const getState = useCallback(() => (isControlled ? values : clapState), [clapState, values, isControlled]);
...
const ClapIcon = ({ style: userStyles = {}, className }) => {
  ...
  return (
    <span>
      <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100.08 125' className={classNames} style={userStyles}>
...
const ClapCount = ({ style: userStyles = {}, className }) => {
  ...
  return (
    <span ref={setRef} data-refkey='countRef' className={classNames} style={userStyles}>
      + {count}
    </span>
  );
};
...
const ClapTotal = ({ style: userStyles = {}, className }) => {
  ...
  return (
    <span ref={setRef} data-refkey='totalRef' className={classNames} style={userStyles}>
      {countTotal}
    </span>
  );
};
...
const Usage = () => {
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      {!!count && <div className={styles.info}>{`You have clapped ${count} ${count === 1 ? "time" : "times"}`}</div>}
    </div>
  );
};
